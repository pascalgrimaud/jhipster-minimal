...
public class UserResource {
    ...
    public ResponseEntity<UserEntity> createUser(@Valid @RequestBody UserDTO userDTO) throws URISyntaxException {
        ...
        if (userDTO.getId() != null) {
            ...
            return ResponseEntity
                .created(new URI("/api/users/" + newUserEntity.getLogin()))
                .headers(
                    HeaderUtil.createAlert(
                        applicationName,
                        "A user is created with identifier " + newUserEntity.getLogin(),
                        newUserEntity.getLogin()
                    )
                )
                .body(newUserEntity);
        }
    ...
    public ResponseEntity<UserDTO> getUser(@PathVariable @Pattern(regexp = Constants.LOGIN_REGEX) String login) {
        ...
        return ResponseUtil.wrapOrNotFound(userApplicationService.getUserWithAuthoritiesByLogin(login).map(UserDTO::new));
    }
